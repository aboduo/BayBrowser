...
- (void)initialize {
    ...
    if (!_indicatorDisabled) [self initalizeImageCounter];
    [self loadData];
}
...
- (void)loadData {
    if (hack == 0) {
        ...
        if ([aImageUrls count] > 0) {
            ...
            UIImageView *blankImage = [[UIImageView alloc] initWithFrame:_scrollView.frame];
            [blankImage setImage:[UIImage imageNamed:@"tpbBig.jpg"]];
            [_scrollView addSubview:blankImage];
        }
    }
}
...
- (void)imageTapped:(UITapGestureRecognizer *)sender {
    if (_delegate && [_delegate respondsToSelector:@selector(imagePager:didSelectImageAtIndex:)]) [_delegate imagePager:self didSelectImageAtIndex:[(UIGestureRecognizer *)sender view].tag];
}
